...
def create_yearly_edge_dfs(df):
    ...
    for year in years_to_process:
        ...
        df_final = df_y[df_y['협력유형4'].astype(str).strip().replace(" ", "") == '법인간'].copy()
...
def analyze_networks(_edge_dfs):
    """
    엣지 DF 딕셔너리를 기반으로 네트워크 지표 및 중심성을 계산합니다.
    경고 메시지는 호출부에서 표기하도록 리스트로 반환합니다.
    """
    overall_metrics = []
    ...
    for year, df_sheet_data in _edge_dfs.items():
        ...
        avg_deg = sum(d for _, d in G.degree()) / num_nodes if num_nodes > 0 else 0
        ...
        if comps:
            ...
            if num_nodes > 0:
                gc_ratio = (gc_nodes / num_nodes) * 100
...
def _aggregate_weighted_edges(df_edges: pd.DataFrame, min_weight: int) -> pd.DataFrame:
    """(source, target) 동일 쌍을 count로 가중치 부여."""
    if df_edges.empty:
...
def _choose_layout(G: nx.Graph, layout_name: str, seed: int = 42) -> dict:
    layout_name = (layout_name or "spring").lower()
...
def _metric_series(df_centrality: pd.DataFrame, metric: str) -> pd.Series:
    if metric == "degree":
        # degree는 연결중심성과 스케일만 다름 → 연결중심성 재사용
        metric = "연결중심성"
    col = metric if metric in df_centrality.columns else "연결중심성"
    s = df_centrality.set_index("기관명")[col]
    # NaN → 0
    return s.fillna(0.0)
...
def visualize_network_plotly_advanced(
    ...
    *,
    ...
    min_degree: int = 0,
    keep_top_n_nodes_by_metric: int = 0,  # 0이면 비활성
    edge_width_range: tuple = (0.5, 6.0),
    edge_opacity: float = 0.35,
    edge_bins: int = 4
) -> go.Figure:
    """
    고급 옵션 기반 Plotly 네트워크 시각화.
    """
    # 1) 에지 가중치 집계 + 필터
    ...
    # 7) 에지 width 계산 (가중치 → 구간(bin) 후 trace 여러 개)
    ...
    bin_idx = np.digitize(widths, bins, right=True) - 1  # 0..edge_bins-1
    ...
    for b in range(edge_bins):
        ...
        for (edge, wid, idx) in zip(G.edges(), widths, bin_idx):
            ...
            this_w = wid  # 대표 width(같은 bin이라 유사)
        if xs:
    ...
    node_trace = go.Scatter(
        x=node_x, y=node_y,
        mode='markers+text' if show_labels != "Off" else 'markers',
        hoverinfo='text',
        ...
        marker=dict(
            ...
            colorbar=dict(
                thickness=16,
                title=dict(text=f'{node_color_metric}', side='right')
            ),
...
def run():
    ...
    if st.session_state.sna_results:
        results = st.session_state.sna_results
        df_metrics = results["df_metrics"]
        df_centralities = results["df_centralities"]
        edge_dfs = results["edge_dfs"]
            ...
            with st.expander("⚙️ 고급 시각화 옵션", expanded=True):
                ...
                with col2:
                    ...
                    color_scale = st.selectbox("Color Scale",
                        ["YlGnBu", "Viridis", "Plasma", "Cividis", "Turbo", "Magma", "Inferno", "RdBu", "Portland"])
                ...
                with col3:
                    ...
                    label_pos = st.selectbox("라벨 위치", ["top center", "middle center", "bottom center", "top left", "top right"])
                ...
                with col5:
                    keep_top_n_nodes = st.number_input("선택한 기준으로 Top-N 노드만 유지(0=해제)", min_value=0, value=0, step=1, format="%d")
