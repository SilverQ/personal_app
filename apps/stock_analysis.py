...
def run():
    ...
    date = st.sidebar.date_input("조회 기준일", value=default_date, max_value=today)

    # 2) 외국인 소진율 범위 슬라이더
    ownership_min, ownership_max = st.sidebar.slider(
        "외국인 소진율 범위 (%)",
        0, 100, (10, 50)
    )

    # 3) 외국인 최근 순매수 종목만 필터
    filter_foreign_buy = st.sidebar.checkbox("외국인 최근 순매수 종목만", value=False)

    # 4) 개인 최근 순매도 종목만 필터
    filter_individual_sell = st.sidebar.checkbox("개인 최근 순매도 종목만", value=False)
